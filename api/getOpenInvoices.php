...
$styleArrayLine = [
	...
	$spreadsheet->getActiveSheet()->setCellValue('I1', "Agency");
	...
	$eft=$X->sql($sql);
		...
		$sql="select * from nua_agency_company where agency_id <> 46 and  company_id = " . $eftRow['company_id'];
		$t=$X->sql($sql);
		$ii=0;
		foreach($t as $y) {
                     $sql="select org_name from nua_agency where id = " . $y['agency_id'];
		     $tt=$X->sql($sql);
		     if ($ii==1) {
                         $cell="J".$row;
                         $spreadsheet->getActiveSheet()->getStyle($cell)->applyFromArray($styleArrayLine);
	                 $spreadsheet->getActiveSheet()->setCellValue($cell, $tt[0]['org_name']);
			 $ii++;
		     } 
		     if ($ii==0) {
                         $cell="I".$row;
